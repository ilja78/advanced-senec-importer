  ...
  def record(row)
    {
      ...
      fields: {
        inverter_power: parse_kwh(row, 'Stromerzeugung [kW]'),
        house_power: parse_kwh(row, 'Stromverbrauch [kW]'),
        bat_power_plus: parse_kwh(row, 'Akkubeladung [kW]', 'Akku-Beladung [kW]'),
        bat_power_minus: parse_kwh(row, 'Akkuentnahme [kW]', 'Akku-Entnahme [kW]'),
        bat_fuel_charge: nil,
        bat_charge_current: parse_a(row, 'Akku Stromstärke [A]'),
        bat_voltage: parse_v(row, 'Akku Spannung [V]'),
        grid_power_plus: parse_kwh(row, 'Netzbezug [kW]'),
        grid_power_minus: parse_kwh(row, 'Netzeinspeisung [kW]'),
